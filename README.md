...
3. Download the [Fashion MNIST Dataset](https://www.kaggle.com/datasets/zalando-research/fashionmnist) and rename `fashion-mnist_test.csv`, and `fashion-mnist_train.csv` as `train.csv`, and `test.csv` respectively, then put two csv files in the newly-created `fashion-mnist` folder under this folder.
